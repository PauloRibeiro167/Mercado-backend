...
RECRIAR_PERMISSOES = ENV['RECRIAR_PERMISSOES'] == 'true'
...
begin
  ActiveRecord::Base.transaction do
    ...
    permissaos.each do |permissao_attrs|
      begin
        ...
        if record.new_record?
          record.save!
          permissaos_criadas += 1
        else
          record.save!
          permissaos_atualizadas += 1
        end
      rescue => e
        erros_ao_criar << { permissao: permissao_attrs[:chave_acao], erro: e.message }
        puts "Erro ao processar permissao '#{permissao_attrs[:chave_acao]}': #{e.message}"
      end
    end
    ...
    if itens_criados == 0 && permissaos_atualizadas == 0
      ...
      puts Rainbow("\nSeed de carga de permissões executada com sucesso!").bold.green
    end
    puts Rainbow("Seed de carga da tabela ").bold.green + Rainbow("Permissaos ").bold.red + Rainbow("concluída com sucesso!").bold.green
  end
rescue => e
